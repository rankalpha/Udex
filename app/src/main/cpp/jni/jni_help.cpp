...
namespace jni {
    ...
    void BypassHiddenApiByRefection(JNIEnv* env) {
        /*
        ScopedLocalRef<jclass> classClass(env, env->FindClass("java/lang/Class"));
        //jmethodID forNameMID = env->GetStaticMethodID(classClass.get(), "forName", "(Ljava/lang/String;)Ljava/lang/Class;");
        //ScopedLocalRef<jstring> vmRuntimeName(env, env->NewStringUTF("dalvik/system/VMRuntime"));
        //ScopedLocalRef<jobject> vmRuntimeClass(env, env->CallStaticObjectMethod(classClass.get(), forNameMID, vmRuntimeName.get()));
        ScopedLocalRef<jclass> vmRuntimeClass(env, env->FindClass("dalvik/system/VMRuntime"));

        jmethodID getRuntime_art_method = env->GetStaticMethodID(vmRuntimeClass.get(),
                                                                    "getRuntime",
                                                                    "()Ldalvik/system/VMRuntime;");
        jobject vmRuntime_instance = env->CallStaticObjectMethod(vmRuntimeClass.get(),
                                                                    (jmethodID) getRuntime_art_method);

        ScopedLocalRef<jstring> getRuntimeMethodName(env, env->NewStringUTF("getRuntime"));
        ScopedLocalRef<jobjectArray> emptyArgs(env, env->NewObjectArray(0, env->FindClass("java/lang/Class"), nullptr));
        ScopedLocalRef<jobject> getRuntimeMethod(env, jni::Java_getDeclaredMethod(env, nullptr, vmRuntimeClass.get(), getRuntimeMethodName.get(), emptyArgs.get()));

        jmethodID r_invokeMID = env->GetMethodID(env->GetObjectClass(getRuntimeMethod.get()), "invoke", "(Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/Object;");
        ScopedLocalRef<jobject> runtimeInstance(env, env->CallObjectMethod(getRuntimeMethod.get(), r_invokeMID, nullptr, nullptr));

        ScopedLocalRef<jstring> setHiddenApiExemptionsName(env, env->NewStringUTF("setHiddenApiExemptions"));

        ScopedLocalRef<jclass> stringArrayClass(env, env->FindClass("[Ljava/lang/String;"));
        ScopedLocalRef<jobjectArray> paramType(env, env->NewObjectArray(1, classClass.get(), nullptr));
        env->SetObjectArrayElement(paramType.get(), 0, stringArrayClass.get());
        ScopedLocalRef<jobject> setHiddenApiExemptionsMethod(env, Java_getDeclaredMethod(env, nullptr, env->GetObjectClass(vmRuntime_instance), setHiddenApiExemptionsName.get(), paramType.get()));

        jmethodID s_invokeMID = env->GetMethodID(env->GetObjectClass(setHiddenApiExemptionsMethod.get()), "invoke", "(Ljava/lang/Object;[Ljava/lang/Object;)Ljava/lang/Object;");

        ScopedLocalRef<jstring> exemption(env, env->NewStringUTF("L"));
        ScopedLocalRef<jclass> stringClass(env, env->FindClass("java/lang/String"));
        ScopedLocalRef<jobjectArray> exemptions(env, env->NewObjectArray(1, stringClass.get(), nullptr));
        env->SetObjectArrayElement(exemptions.get(), 0, exemption.get());
        ScopedLocalRef<jobjectArray> args(env, env->NewObjectArray(1, env->FindClass("java/lang/Object"), nullptr));
        env->SetObjectArrayElement(args.get(), 0, exemptions.get());
        //ScopedLocalRef<jobjectArray> s_paramType(env, env->NewObjectArray(2, env->FindClass("java/lang/Object"), nullptr);
        //env->SetObjectArrayElement(s_paramType.get(), 0, runtimeInstance.get());
        //env->SetObjectArrayElement(s_paramType.get(), 1, args.get());

        env->CallVoidMethod(setHiddenApiExemptionsMethod.get(), s_invokeMID, vmRuntime_instance, args.get());
        if (env->ExceptionOccurred()) {
            env->ExceptionDescribe();
            env->ExceptionClear();
            LOGD("Exception occurred while calling setHiddenApiExemptions");
            return;
        }
        LOGD("Successfully unsealed process");
         */
    }

    void BypassHiddenApiByRefection1(JNIEnv* env) {
        JavaVM *javaVM;
        env->GetJavaVM(&javaVM);

        ScopedLocalRef<jclass> classClass(env, env->FindClass("java/lang/Class"));
        jmethodID forName = env->GetStaticMethodID(classClass.get(), "forName", "(Ljava/lang/String;)Ljava/lang/Class;");
        jmethodID getDeclaredMethod = env->GetMethodID(classClass.get(), "getDeclaredMethod", "(Ljava/lang/String;[Ljava/lang/Class;)Ljava/lang/reflect/Method;");
        //ScopedLocalRef<jstring> vmRuntimeName(env, env->NewStringUTF("dalvik/system/VMRuntime"));
        //ScopedLocalRef<jobject> vmRuntimeClass(env, env->CallStaticObjectMethod(classClass.get(), forName, vmRuntimeName.get()));

        ScopedLocalRef<jclass> vmRuntimeClass(env, env->FindClass("dalvik/system/VMRuntime"));
        jmethodID getRuntime = env->GetStaticMethodID(vmRuntimeClass.get(), "getRuntime", "()Ldalvik/system/VMRuntime;");
        ScopedLocalRef<jobject> runtimeInstance(env, env->CallStaticObjectMethod(vmRuntimeClass.get(), getRuntime));

        /**/
        //ScopedLocalRef<jstring> getRuntimeName(env, env->NewStringUTF("getRuntime"));
        //ScopedLocalRef<jobject> getRuntimeMethod(env, env->CallObjectMethod(vmRuntimeClass.get(), getDeclaredMethod, getRuntimeName.get(), NULL));
        ScopedLocalRef<jclass> stringArrayClass(env, env->FindClass("[Ljava/lang/String;"));
        ScopedLocalRef<jobjectArray> paramType(env, env->NewObjectArray(1, classClass.get(), nullptr));
        env->SetObjectArrayElement(paramType.get(), 0, stringArrayClass.get());
        ScopedLocalRef<jstring> setHiddenApiExemptionsName(env, env->NewStringUTF("setHiddenApiExemptions"));
        ScopedLocalRef<jobject> setHiddenApiExemptionsMethod(env, env->CallObjectMethod(vmRuntimeClass.get(), getDeclaredMethod, setHiddenApiExemptionsName.get(), paramType.get()));
        //ScopedLocalRef<jobject> setHiddenApiExemptionsMethod_(env, Java_getDeclaredMethod(env, nullptr, vmRuntimeClass.get(), setHiddenApiExemptionsName.get(), paramType.get()));
        //ScopedLocalRef<jobject> runtimeInstance(env, env->CallObjectMethod(vmRuntimeClass.get(), (jmethodID)getRuntimeMethod.get()));
        jmethodID setHiddenApiExemptions = env->FromReflectedMethod(setHiddenApiExemptionsMethod.get());
        /**/

        //jmethodID setHiddenApiExemptions = env->GetMethodID(vmRuntimeClass.get(), "setHiddenApiExemptions", "([Ljava/lang/String;)V");
        if (setHiddenApiExemptions) {
            ...
            env->CallVoidMethod(runtimeInstance.get(), setHiddenApiExemptions, jarray.get());
        }
        else {
        ...
        const char* target_char = "L";
        ScopedLocalRef<jstring> mystring(env, env->NewStringUTF(target_char));
        ScopedLocalRef<jclass> stringArrayClass(env, env->FindClass("java/lang/String"));
        ScopedLocalRef<jobjectArray> paramTypes(env, env->NewObjectArray(1, stringArrayClass.get(), nullptr));
        env->SetObjectArrayElement(paramTypes.get(), 0, mystring.get());
    ...
    bool MergeDexAndSoToClassLoader(JNIEnv *env, const char *dex_path, const char *so_dir) {
        ...
        if (api_level > __ANDROID_API_L_MR1__) {
            ...
            element_array_obj = CallStaticMethod(env, pathList_clazz.get(), "makeDexElements",
                                                 ...
                                                 dex_path_arrayList_obj,
                                                 nullptr, nullptr, classloader_obj.get());
        } else {
            element_array_obj = CallStaticMethod(env, pathList_clazz.get(), "makeDexElements",
