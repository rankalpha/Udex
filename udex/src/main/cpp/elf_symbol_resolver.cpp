...
static std::vector<RuntimeModule> &get_process_map_with_proc_maps() {
    ...
    while (!feof(fp)) {
        ...
        if (sscanf(line_buffer,
                   ...
                   &region_start, &region_end, permissions, &region_offset, &dev_major, &dev_minor, &inode, &path_index) < 7) {
        ...
        if (strcmp(permissions, "r--p") != 0 && strcmp(permissions, "r-xp") != 0) continue;
    ...
    // Handle dynamic segment
    {
        ...
        for (size_t i = 0; i < ehdr->e_phnum; i++) {
            if (phdr[i].p_type == PT_DYNAMIC) {
                ...
                if (ctx->load_bias == 0) ctx->load_bias = ehdr_addr - (phdr[i].p_vaddr - phdr[i].p_offset);
...
void *(*solist_get_head)() = (void *(*)())resolve_elf_internal_symbol(get_android_linker_path(), "__dl__Z15solist_get_headv");
...
void *resolve_elf_internal_symbol(const char *library_name, const char *symbol_name) {
    ...
    if (module.load_address) {
        ...
        if (result) result = (void *)((addr_t)result + (addr_t)module.load_address - ((addr_t)mmap_buffer - (addr_t)ctx.load_bias));
    }
